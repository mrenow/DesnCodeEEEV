...
send_addr;(A1 = char addr, A2 = char next_state)  A3 dirty
				STR LR, [SP, #-4]! ; Full descending 
				
				; Send start bit
				MOV A3, #STA
				STRB A3, [V1, #I20CONSET]
				
				BL resume
				
				MOV A3, #STA
				STRB A3, [V1, #I20CONCLR]; Reset Start bit
...
send_data;(A1 = char data, A2 = char next_state) A3 dirty. MUST Be called in main code block
				
				STR LR, [SP, #-4]! ; Full descending
				STRB A1, [V1, #I20DAT]; Load data into dat
				BL resume
	
				LDR A3, [V1, #I20STAT] ; Check for error
				TEQ A3, A2; If not in expected state
				BNE restart_transmission; Transmission restart needed
				
				LDR LR, [SP], #4 ; Full descending 
				MOV PC, LR
				
restart_transmission; MUST be called one call down from main code block.
				
				MOV A3, #STO
				STRB A3, [V1, #I20CONSET]; Stop transmission
